...
public class StatMonthDAO{
	...
	private static StatMonthDAO instance;
	public static StatMonthDAO getInstance() {
		if (instance == null) {
			instance = new StatMonthDAO();
		}
		return instance;
	}
	...
	public ArrayList<StatisticsDTO> ListView() {
		...
		try {
			...
			sql.append(" (SELECT TO_CHAR(IN_TIME,'yyyy-mm') TIME, ");
			sql.append(" COUNT(CASE WHEN MONTH_NUM != 0 THEN 1 END) IN_MONTH, ");
			sql.append(" COUNT(CASE WHEN MONTH_NUM = 0 THEN 1 END) IN_NOMAL ");
			sql.append(" FROM PMS_LOG GROUP BY TO_CHAR(IN_TIME,'yyyy-mm') ");
			sql.append(" ) A LEFT JOIN") ;
			sql.append(" (SELECT TO_CHAR(OUT_TIME,'yyyy-mm') TIME, ");
			...
			sql.append(" FROM PMS_LOG GROUP BY TO_CHAR(OUT_TIME,'yyyy-mm') ");
			sql.append(" ) B ON A.TIME = B.TIME LEFT JOIN ");
			sql.append(" (SELECT TO_CHAR(SDATE,'yyyy-mm') TIME, COUNT(*) COUNT, SUM(MONTH_PAY) MONTH_PAY ");
			sql.append(" FROM PMS_MONTH_MEMBER GROUP BY TO_CHAR(SDATE,'yyyy-mm') ");
			sql.append(" ) C ON A.TIME = C.TIME");
			sql.append(" WHERE SUBSTR(A.TIME,0,4)='"+format_time+"' ORDER BY A.TIME");
