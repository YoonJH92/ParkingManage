...
public class PMSRandom {
	...
	public static String randomCouponName() {
		List<String> cname = Arrays.asList("1주년기념", "10주년기념", "건물주탄신일", "생일이니까 하나준다", "기부해준다", "영화보고옴", "제휴식당갔다옴",
				"건물내 마트이용", "월정액", "월정액", "월정액", "월정액");
		Collections.shuffle(cname);
		return cname.get(0);
	}

	public static int randomCouponDay() {
		List<Integer> daynum = Arrays.asList(1, 10, 30, 100);
	...
	public static String randomCCode() {
		List<String> name = Arrays.asList("a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o",
	...
	private void randomCP(int num) {
		...
		for (int i = 0; i < num; i++) {
			...
			int daynum = randomCouponDay();
			int dpay = randomCouponDPay();
			String purpose = randomCouponPurpose();
			String ccode = randomCCode();
			RandomInsert randomInsert = new RandomInsert();
			randomInsert.randomCoupon(cname, daynum, dpay, purpose, ccode);
